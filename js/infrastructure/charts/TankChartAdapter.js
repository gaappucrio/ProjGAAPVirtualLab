        ...
        options: {
            ...
            plugins: {
                tooltip: {
                    ...
                    padding: 10,
                    callbacks: {
                        title: (context) => `Tempo: ${context[0].label}s`,
                        label: (context) => `Volume: ${toDisplayValue('volume', context.parsed.y).toFixed(1)} ${getUnitSymbol('volume')}`
                    }
                }
            },
            scales: {
...
export function refreshEmptyMonitorChartPresentation(chart) {
    ...
    chart.data.datasets[0].label = 'Selecione um Tanque ou Bomba';
    ...
    chart.options.plugins.tooltip.callbacks.label = (context) =>
        `Volume: ${toDisplayValue('volume', context.parsed.y).toFixed(1)} ${getUnitSymbol('volume')}`;
